...
//        System.out.println(service.getUserList().);
    ...
    /** User登録画面を表示 */
    @GetMapping("/register")
    public String getRegister(@ModelAttribute User user) { //@ModelAttributeアノテーションを付与すると、自動的に（テンプレートにデータを受け渡す）Modelにインスタンスが登録されます。これは@ModelAttributeアノテーションを付与せずに model.addAttribute("user", user); と記述することと同一です。
        // User登録画面に遷移
